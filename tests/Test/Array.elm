...
tests =
    let
        ...
        runtimeCrashTests =
            suite "Conditions which caused old implementation to crash at runtime"
                [ test "magic slice"
                    <| let
                        n =
                            10
                ...
                , test "magic slice 2"
                    <| let
                        ...
                        res =
                            Array.append (Array.slice 1 32 ary) (Array.slice (32 + 1) -1 ary)
                       in
                        assertEqual res res
                , test "magic append"
                    <| let
                        res =
                            Array.append (Array.initialize 1 (always 1))
                                (Array.initialize (32 ^ 2 - 1 * 32 + 1) (\i -> i))
                       in
                        assertEqual res res
                ]

        nativeTests =
            suite "Conversion to JS Arrays"
