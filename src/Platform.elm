...
The first argument, `init` is a Tuple that lets you specify how your program should start.
In that Tuple, its first value is the initial state of your program's model.
The second value is an initial command. Often that command is `Cmd.none`.

### update

The second argument, `update` is a function that takes the following arguments:
...
The `update` function must return a Tuple containing the modified form of your program's model,
and a command. The commad should be generated by passing data through an outbound port.

### subscriptions

The third argument, `subscriptions` is a function that accepts your program's model, and returns a `Sub`.
As explained in [the effects section of the Elm language guide](https://guide.elm-lang.org/architecture/effects/),
it "declares any event sources you need to subscribe to given the current model."
Your model may not be used in your definition of the `subscriptions` function.
...
It's possible that the `subscriptions` function _could_ return differing `Sub`s based on
...
type alias Model =
    { numPasswordChanges : Int}
-- Assume the update function increments
-- `numPasswordChanges`.

subscriptions : Model -> Sub Msg
subscriptions model =
  if model.numPasswordChanges > 100000 then
    Sub.none
  else
    changePwPort ChangePassword
```

To specify multiple subscriptions, use [Sub.batch](http://package.elm-lang.org/packages/elm-lang/core/5.0.0/Platform-Sub#batch).
-}
program
  : { init : (model, Cmd msg)
